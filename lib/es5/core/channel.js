...
var errChannelWriterEmpty = new exception_1.Exception('Writer empty');
var errChannelReaderEmpty = new exception_1.Exception('Reader empty');
/**
 * Thrown when case is not writable or not ready
 */
exports.errChanneWriteCase = new exception_1.Exception('write case not ready');
/**
 * Thrown when case is not readable or not ready
 */
exports.errChanneReadCase = new exception_1.Exception('read case not ready');
/**
...
function selectChan() {
    ...
    // 存在多個 case
    return new Promise(function (resolve, reject) {
        var conns = new Array(cases.length);
        for (var i = 0; i < cases.length; i++) {
            conns[i] = cases[i].do(function (c) {
