...
exports.defaultOutput = {
    log: function (opts, vals) {
        ...
        if (opts.time) {
            var d = new Date();
            var str = "[".concat(d.getFullYear(), "/").concat(pad(d.getMonth() + 1, 2), "/").concat(pad(d.getDate(), 2), " ").concat(pad(d.getHours(), 2), ":").concat(pad(d.getMinutes(), 2), ":").concat(pad(d.getSeconds(), 2), "]");
            if (prefix == '') {
                prefix = str;
            }
            else {
