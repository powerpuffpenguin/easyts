import { ValueCallback } from '../types.ts';
import { Chan, ReadChannel } from '../channel.ts';
 ...
 * @throws {@link TimeException} if ms <= 0
 */
export function tick(ms: number): ReadChannel<Date> {
    if (!Number.isSafeInteger(ms) || ms < 1) {
 ...
 * @throws {@link TimeException} if ms <= 0
 */
export function tickFunc(ms: number, callback: ValueCallback<Date>): Ticker {
    if (!Number.isSafeInteger(ms) || ms < 1) {
        throw new TimeException('non-positive interval for tick')
    }
